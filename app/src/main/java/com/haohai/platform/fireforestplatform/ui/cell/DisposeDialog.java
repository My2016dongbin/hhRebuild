...
import com.haohai.platform.fireforestplatform.databinding.DialogDisposeBinding;
...
public class DisposeDialog extends Dialog {

    private final Context context;
    private DisposeDialogListener dialogListener;
    private final DialogDisposeBinding binding;
    private DisposeResult disposeResult;
    private boolean isReal = false;

    public DisposeDialog(@NonNull Context context, @StyleRes int themeResId) {
        super(context, themeResId);
        this.context = context;
        binding = DataBindingUtil.inflate(LayoutInflater.from(getContext()), R.layout.dialog_dispose, null, false);
    ...
    private void bind_() {
        binding.right.setOnClickListener(v -> {
            if(binding.remarkView.getText().toString().isEmpty()){
                Toast.makeText(context, "请输入处置上报内容", Toast.LENGTH_SHORT).show();
                return;
            }
