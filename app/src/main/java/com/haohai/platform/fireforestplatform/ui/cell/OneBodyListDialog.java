...
public class OneBodyListDialog extends Dialog implements OneBodyFireViewBinder.OnItemClickListener {
    ...
    private void filterData() {
        ...
        filterList.addAll(oneBodyFireList);
    ...
    private int filterState = 1;//0全部  1未处置  2处置中  3处置完毕
    private void bind_() {
    ...
    private void updateFilterState() {
        ...
        if(filterState == 1){
            binding.filterText.setText("未处置");
        }
        if(filterState == 2){
            binding.filterText.setText("处置中");
        }
        if(filterState == 3){
            binding.filterText.setText("处置完毕");
        }
    }
