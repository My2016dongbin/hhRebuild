...
public class ResourceDetailDialog extends Dialog implements INaviInfoCallback {
    ...
    private void bind_() {
        binding.edit.setOnClickListener(v -> {
            editRes(resource.getResourceType(),resource.getId(),resource.getObj(),resource.getTypes().replace("/api/",""));
        });
        binding.delete.setOnClickListener(v -> {
            deleteRes(resource.getResourceType(),resource.getId(),resource.getTypes().replace("/api/",""));
        });
