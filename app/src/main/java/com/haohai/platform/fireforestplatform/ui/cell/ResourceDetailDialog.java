...
public class ResourceDetailDialog extends Dialog implements INaviInfoCallback {
    ...
    private void updateData() {
        ...
        try{
            binding.lngLat.setText(parse10(resource.getLongitude())+","+parse10(resource.getLatitude()));
        }catch (Exception e){
    ...
    private void guide() {
        ...
        double[] doubles_end = LatLngChangeNew.calWGS84toGCJ02(Double.parseDouble(resource.getLatitude()), Double.parseDouble(resource.getLongitude()));
        LatLng latLng_end = new LatLng(doubles_end[0],doubles_end[1]);


        Poi start = new Poi("", latLng_start, "");
        Poi end = new Poi(resource.getResourceName(), latLng_end, "");
