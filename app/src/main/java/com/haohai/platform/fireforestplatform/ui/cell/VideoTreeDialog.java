...
public class VideoTreeDialog extends Dialog implements GridTreesViewBinder.OnItemClickListener, GridModelViewBinder.OnItemClickListener, GridCameraViewBinder.OnItemClickListener {
    ...
    private void bind_() {
        binding.rlv.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                viewModel.offset = recyclerView.computeVerticalScrollOffset();
                HhLog.e("offset OnScrollListener " + recyclerView.computeVerticalScrollOffset());
    ...
    public void onItemClick(GridTrees gridTrees, boolean state) {
        if(viewModel.tabStatus==0){
            ...
            HhLog.e("offset " + offset + " , " + binding.rlv.getHeight());
            viewModel.updateData();
            /*new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
//                    binding.rlv.offsetTopAndBottom(0);
                    *//*if(offset > binding.rlv.getHeight() / 4){
                        binding.rlv.offsetTopAndBottom(binding.rlv.getHeight() / 4);
                    }else{
//                        binding.rlv.offsetChildrenVertical(-1* offset);
                        binding.rlv.offsetTopAndBottom(offset);
                    }*//*
                }
            },2);*/
        }else if(viewModel.tabStatus==1){
...
//                        binding.rlv.offsetChildrenVertical(-1* offset);
    ...
    public void onItemClick(GridCamera gridCamera) {
        ...
        viewModel.getStream(gridCamera.getId(), gridCamera.getMonitorId(), gridCamera.getId(),gridCamera.getCameraType(),gridCamera.getDeviceId(),gridCamera.getSerial());
    ...
    public void onStarClick(GridCamera gridCamera) {
        ...
        if(viewModel.tabStatus==0){
            ...
            int offset = binding.rlv.computeVerticalScrollOffset();
            HhLog.e("offset " + offset + " , " + binding.rlv.getHeight());//TODO E/dancer: offset 454 , 1845
            viewModel.updateData();
            /*new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
