...
public class FgMapViewModel extends BaseViewModel {
    ...
    public void initSheQuGrid(){
        //社区网格数据json解析
        try {
            JSONObject jsonObject = new JSONObject(new GetJsonDataUtil().getJson(context,"sheQu2.json"));
    ...
    public void getResourceTypeData() {
        ...
        HhHttp.postString()
                ...
                .execute(new LoggedInStringCallback(this, context) {
                    @Override
                    public void onSuccess(String response, int id) {
                        HhLog.e("resourceType response old " + response);
                        try {
                            JSONObject jsonObject = new JSONObject(response);
                            JSONArray data = jsonObject.getJSONArray("data");
                            ...
                            HhHttp.postString()
                                    ...
                                    .execute(new LoggedInStringCallback(FgMapViewModel.this, context) {
                                        @Override
                                        public void onSuccess(String response, int id) {
                                            HhLog.e("resourceType response new " + response);
                                            try {
                                                JSONObject jsonObject = new JSONObject(response);
                                                JSONArray data = jsonObject.getJSONArray("data");
                                                List<ResourceType> value = resourceTypeList.getValue();
    ...
    public void clickModelShequ(SheQu sheQu,boolean state) {
        if(state){
            ...
            //获取选中数据
            sheQuCurrentList.postValue(sheQu.getGeometry().getCoordinates().get(0));
        }else{
            //移除选中数据
            sheQuCurrentList.postValue(new ArrayList<>());
        }
        ...
        for (int i = 0; i < value.size(); i++) {
            ...
            if(Objects.equals(type.getProperties().getNAME(), sheQu.getProperties().getNAME())){
                value.get(i).setChecked(state);
    ...
    public void updateResEventAdding(ResourceType resourceType,String name) {
        //移除选中类型数据
        removeResourceByApiUrl(resourceType);
        //获取选中类型数据
        postResourceByApiUrlAdding(resourceType,name);
        List<ResourceType> value = resourceTypeList.getValue();
        for (int i = 0; i < value.size(); i++) {
            ResourceType type = value.get(i);
            if(Objects.equals(type.getId(), resourceType.getId())){
    ...
    public void removeResourceByApiUrl(ResourceType resourceType) {
        HhLog.e("removeResourceByApiUrl " + resourceType.getApiUrl() + " , " + resourceType.getCode());
        ...
        if(value!=null && !value.isEmpty()){
            ...
            for (Resource res:value) {
                //隔离带排除隔离网2干扰
                if(Objects.equals(resourceType.getCode(), "isolationBelt")){
                    if(Objects.equals(res.getApiUrl(), "/api/isolationBelt") && Objects.equals(res.getType(), "2")){
                        continue;
                    }
                }
                //隔离网排除隔离带1干扰
                if(Objects.equals(resourceType.getCode(), "isolationNet")){
                    if(Objects.equals(res.getApiUrl(), "/api/isolationBelt") && Objects.equals(res.getType(), "1")){
                        continue;
                    }
                }
                if(Objects.equals(res.getApiUrl(), "/api/"+resourceType.getCode()) || Objects.equals(res.getApiUrl(), resourceType.getApiUrl()) || Objects.equals(res.getResourceType(), resourceType.getCode()) || (res.getResourceType()==null||res.getResourceType().isEmpty())){
                    value_C.remove(res);
                }
            }
    ...
    public void postResourceByName(String name) {
        ...
        HhHttp.get()
                ...
                .execute(new LoggedInStringCallback(this, context) {
                    @Override
                    public void onSuccess(String response, int id) {
                        HhLog.e("postResourceByName " + URLConstant.GET_RESOURCE_SEARCH + " , " + response);
                        try {
                            loading.postValue(new LoadingEvent(false, ""));
                            JSONObject jsonObject = new JSONObject(response);
                            JSONArray data = jsonObject.getJSONArray("data");
                            ...
                            for (int i = 0; i < data.length(); i++) {
                                ...
                                resource.setObj(obj);
                                resource.setTypes(resource.getResourceType());
                                list.add(resource);
                            }
                            if(!list.isEmpty()){
                                ...
                                try{
                                    EventBus.getDefault().post(new Search(Double.parseDouble(res.getPosition().getLat()),Double.parseDouble(res.getPosition().getLng())));
                                }catch (Exception e){
                                    //
                                }
    ...
    public void postResourceByApiUrl(ResourceType resourceType) {
        ...
        try {
            ...
            if(Objects.equals(resourceType.getCode(), "isolationNet")){
                resourceType.setApiUrl("/api/isolationBelt");
                dto.put("type",2);
                    ...
                    public void onSuccess(String response, int id) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < data.length(); i++) {
                                ...
                                Resource resource = new Gson().fromJson(obj.toString(), Resource.class);
                                resource.setObj(obj);
                                resource.setTypes(resourceType.getApiUrl().replace("/api/",""));
                                resource.setResourceType(resourceType.getApiUrl().replace("/api/",""));
                                list.add(resource);
                            }
                            if(!list.isEmpty()){
                                for (Resource res:list) {
    ...
    public void postResourceByApiUrlAdding(ResourceType resourceType,String name) {
        loading.setValue(new LoadingEvent(true, "正在获取资源信息.."));
        final JSONObject jsonObject = new JSONObject();
        JSONObject dto = new JSONObject();
        try {
        ...
        HhHttp.postString()
                ...
                .execute(new LoggedInStringCallback(this, context) {
                    @Override
                    public void onSuccess(String response, int id) {
                        HhLog.e(resourceType.getApiUrl() + " , adding " + response);
                        try {
                            loading.postValue(new LoadingEvent(false, ""));
                            JSONObject jsonObject = new JSONObject(response);
                            JSONArray data = jsonObject.getJSONArray("data");
                            ...
                            for (int i = 0; i < data.length(); i++) {
                                ...
                                if(Objects.equals(resource.getName(), name)){
                                    HhLog.e("adding " + resource.getName() + " , " + name);
                                    list.add(resource);
                                    CommonData.lngAdding = Double.parseDouble(resource.getPosition().getLng());
                                    CommonData.latAdding = Double.parseDouble(resource.getPosition().getLat());
                                    CommonData.longAdding = new Date().getTime();
                                }
                            }
                            if(!list.isEmpty()){
                                for (Resource res:list) {
