...
public class FgMapViewModel extends BaseViewModel {
    ...
    public final MutableLiveData<List<ArrayList<Double>>> gridCurrentList = new MutableLiveData<>();
    public final MutableLiveData<List<List<ArrayList<Double>>>> multiGridCurrentList = new MutableLiveData<>();
    ...
    public OneBodyFire oneBodyFire;
    public SatelliteFire satelliteFire;
        ...
        HhHttp.get()
            ...
            public void onSuccess(String response, int id) {
                try {
                    ...
                    if(data.length()>0){
                        gridGridList.postValue(new Gson().fromJson(String.valueOf(data),
    ...
    public void clickModelGrid(com.haohai.platform.fireforestplatform.ui.multitype.Grid grid,boolean state) {
        if(state){
            //移除选中数据
            gridCurrentList.postValue(new ArrayList<>());
            multiGridCurrentList.postValue(new ArrayList<>());
            //获取选中数据
            HhHttp.get()
                    .url(URLConstant.GET_GRID_LINE)
                    .addParams("mapType",grid.getGridLevel())
                    .addParams("gridNo",grid.getGridNo())
                    .build().execute(new LoggedInStringCallback(this,context) {
                @Override
                public void onSuccess(String response, int id) {
                    try {
                        ...
                        if(data.length()>0){
                            JSONObject obj = (JSONObject) data.get(0);
                            String areasPoint = obj.getString("areasPoint");
                            areasPoint = areasPoint.replace("\\","");
                            //HhLog.e("areasPoint -" + areasPoint);
                            JSONObject object = new JSONObject(areasPoint);
                            String type = object.getString("type");
                            if("LineString".equals(type)){
                                JSONArray coordinates = object.getJSONArray("coordinates");
                                gridCurrentList.postValue(new Gson().fromJson(String.valueOf(coordinates),
                                        new TypeToken<List<ArrayList<Double>>>(){}.getType()));
                            }else if("MultiLineString".equals(type)){
                                JSONArray coordinates = object.getJSONArray("coordinates");
                                multiGridCurrentList.postValue(new Gson().fromJson(String.valueOf(coordinates),
                                        new TypeToken<List<List<ArrayList<Double>>>>(){}.getType()));
                            }
                        }
                    } catch (JSONException e) {
                        e.printStackTrace();
        ...
        List<com.haohai.platform.fireforestplatform.ui.multitype.Grid> value = gridGridList.getValue();
        for (int i = 0; i < value.size(); i++) {
            com.haohai.platform.fireforestplatform.ui.multitype.Grid type = value.get(i);
            if(Objects.equals(type.getGridNo(), grid.getGridNo())){
                value.get(i).setChecked(state);
                gridGridList.postValue(value);
                if(!state){
                    return;
                }
            }else{
